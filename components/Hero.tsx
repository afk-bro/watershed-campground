...
export default function Hero({ title, subtitle, imageSrc, cta, align = "center" }: Props) {
  ...
  return (
    <section className="relative h-[55vh] min-h-[400px] w-full overflow-hidden">
      ...
      {/* Base Linear Gradient (top → bottom) - adds overall depth */}
      <div
        className="absolute inset-0"
        style={{
          background: 'linear-gradient(to bottom, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.15) 70%, rgba(0,0,0,0.25) 100%)'
        }}
      />
        ...
        <Container>
          <div className={`flex h-full ${textAlign} justify-center`}>
            <div className="relative max-w-xl z-10">
              ...
              {subtitle && <p className="mt-4 text-lg sm:text-xl text-accent-beige/95 leading-relaxed" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.75), 0 4px 8px rgba(0,0,0,0.45)' }}>{subtitle}</p>}
